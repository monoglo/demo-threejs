...
export default {
  ...
  methods: {
    initThreejs() {
      ...
      loader.load(
        `${process.env.BASE_URL}models/tube_re.gltf`,
        gltf => {
          // console.info(gltf)
    ...
    render() {
      ...
      if (this.scene.getObjectByName('箭头')) {
        this.scene.getObjectByName('箭头').position.x += 0.02
        this.arrowMoved += 0.02
        if (this.arrowMoved >= 2) {
          this.arrowMoved = 0
          this.scene.getObjectByName('箭头').position.x -= 2
        }
      }
      if (this.lableDiv) {
        if (this.count > 120) {
      ...
      if (this.famenzhuan) {
        this.scene.getObjectByName('valve').rotation.z += 0.1
      }
